...
def n_long_words(words, n):
    ...
    for word in words:
        if len(word) > n:
            longer_than_n_words.append(word)

    return longer_than_n_words
...
def join_strings_with_comma(words):
    ...
    if len(words) > 1:
        for i in range(len(words)-1):
            new_string = new_string + str(words[i]) + ", "
        new_string = new_string + words[-1]
    else:
        new_string = new_string + str(words[-1])
    # for i in range(len(words)):
    #     word = str(words[i])
...
def duplicates(items):
    ...
    for item in items:
        ...
        for i in range(len(items)):
            if item == items[i]:
                count +=1
                if count > 2 and item not in dups:
                    dups.append(item)
